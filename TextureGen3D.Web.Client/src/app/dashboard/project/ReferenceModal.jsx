...
export default function ReferenceModal({ reference, projectId, token, imageModels, mode, onClose, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const api = ProjectReferences({ token });
      const res = await api.saveGenerated(projectId, {
        imageBase64: base64,
        mode: mode === 'edit' ? 'replace' : 'new',
        referenceId: mode === 'edit' ? reference.id : '',
      });
      if (res.data?.success) {
        onSaved?.();
        onClose();
  ...
  const refThumbUrl = reference
    ? ProjectReferences({ token }).thumbUrl(projectId, reference.id)
    : '';
      ...
      <div className="space-y-4">
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1 flex flex-col">
            <div
              className="flex-1 rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-600 flex items-center justify-center min-h-[300px]"
              style={{ maxHeight: 500 }}
            >
              {generating ? (
